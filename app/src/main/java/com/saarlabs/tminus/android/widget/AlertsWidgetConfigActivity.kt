...
private fun AlertsWidgetConfigScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.widget_alerts_configure_title)) },
                actions = {
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.widget_alerts_lines_label),
                style = MaterialTheme.typography.titleSmall,
                modifier = Modifier.padding(top = 12.dp, bottom = 4.dp),
            )
            for (line in AlertsWidgetLines.ALL) {
                val checked = line.id in selectedIds
                Row(
                    modifier =
                        Modifier.fillMaxWidth()
                            .toggleable(
                                ...
                                onValueChange = { isChecked ->
                                    if (isChecked) {
                                        if (line.id !in selectedIds) selectedIds.add(line.id)
                                    } else {
